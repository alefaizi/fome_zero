...
#==================================================================
# Funções - Countries Page
#==================================================================

# Função para plotar o gráfico de restaurantes por país
def rest_by_country(df1):
...
# Função para plotar o gráfico de cidades por país
...
# Função para plotar o gráfico de média de avaliações por país
def avg_votes_country(df1):
    df_aux = round(df1.loc[:, ['votes', 'country_name']]
                      .groupby('country_name')
                      .mean()
...
# Função para plotar o gráfico com a média do preço em dólar de um prato para dois por país
def avg_cost_for_two(df1):
    df_aux = round(df1.loc[:, ['average_cost_in_usd', 'country_name']]
                      .groupby('country_name')
                      .mean()
                      .sort_values('average_cost_in_usd', ascending=False)
                      .reset_index(), 2)
    fig = px.bar(df_aux, x='country_name', y='average_cost_in_usd', text_auto=True, labels={"country_name": "Países", "average_cost_in_usd": "Preço médio do prato para dois"})
    fig.update_traces(textfont_size=13, textangle=0, textposition='outside', cliponaxis=False)
    return fig
...
# Layout da página
#==================================================================
st.header('Fome Zero - Countries Page')
st.subheader('Confira informações sobre os países presentes na plataforma')
st.markdown("""___""")

with st.container():
    st.markdown('##### Quantidade de restaurantes por país')
    fig = rest_by_country(df1)
...
with st.container():
    ...
    with col1:
        st.markdown('##### Média das avaliações por país')
        fig = avg_votes_country(df1)
        st.plotly_chart(fig, use_container_width=True)
        
    with col2:
        st.markdown('##### Média de um prato para dois por país em USD')
        fig = avg_cost_for_two(df1)
        st.plotly_chart(fig, use_container_width=True)